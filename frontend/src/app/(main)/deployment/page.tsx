...
function EnrollmentTokensTab() {
  ...
  const loadCommands = async (id: string) => {
    try {
      const cmds = await api.enrollment.tokens.commands(id);
      setCommands(cmds);
    } catch {}
  };
    ...
    <div className="space-y-4">
        ...
        {commands ? (
          <div className="space-y-3">
            <CommandBlock
              ...
              icon={<MonitorIcon style={{ width: '12px', height: '12px' }} />}
              command={commands.windows_powershell}
              onCopy={() => copy(commands.windows_powershell, 'windows')}
              copied={copiedField === 'windows'}
            />
            <CommandBlock
              label="Linux (one-liner)"
              icon={<Terminal style={{ width: '12px', height: '12px' }} />}
              command={commands.linux_oneliner ?? commands.linux_bash}
              onCopy={() => copy(commands.linux_oneliner ?? commands.linux_bash, 'linux')}
      ...
      <Card>
        ...
        {loading ? (
          <div className="space-y-2">
            {[...Array(2)].map((_, i) => (
              <div key={i} className="bg-reap3r-surface border border-reap3r-border rounded-xl h-14 animate-pulse" />
            ))}
          </div>
        ) : tokens.length === 0 ? (
          ...
          <div className="space-y-2">
            {tokens.map((token) => (
              <div
                key={token.id}
                className="flex items-center gap-3 px-4 py-3 bg-reap3r-surface/60 border border-reap3r-border/60 rounded-xl group hover:border-reap3r-border-light transition-all"
              >
                ...
                <Badge variant="default">
                  {token.use_count}
                  {token.max_uses > 0 ? `/${token.max_uses}` : ''} uses
                </Badge>
                {token.revoked ? (
                  <Badge variant="danger">Revoked</Badge>
                ) : (
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => copy(token.token, token.id)} className="p-1.5 text-reap3r-muted hover:text-white hover:bg-reap3r-hover rounded-lg transition-all">
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="Production, Lab, Client..."
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button onClick={createToken} disabled={!newTokenName.trim() || creating}>
              Create
            </Button>
          </div>
        </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-[10px] text-reap3r-muted font-bold uppercase tracking-[0.08em] flex items-center gap-1.5">
          {icon} {label}
        </span>
        <button onClick={onCopy} className="text-[10px] text-reap3r-light hover:text-white flex items-center gap-1 transition-colors">
          {copied ? (
            <>
              <Check className="text-reap3r-success" style={{ width: '10px', height: '10px' }} /> Copied
            </>
          ) : (
            <>
      ...
      <pre className="code-block px-4 py-3 text-[11px] overflow-x-auto whitespace-pre-wrap break-all">{command}</pre>
    </div>
  );
}
