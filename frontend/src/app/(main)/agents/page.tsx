...
export default function AgentsPage() {
  ...
  useEffect(() => { fetchAgents(); fetchManifest(); }, [fetchAgents]);
  ...
  const handleUpdateSelected = async (force = false) => {
    ...
    try {
      ...
      toast.success('Update queued', `${res.total} agent(s) → v${res.version}`);
  ...
  const handleDeleteSelected = async () => {
    ...
    for (const id of selectedIds) {
      try { await api.agents.delete(id); deleted++; } catch {}
    }
          ...
          <div className="flex items-center justify-between px-5 py-3 bg-reap3r-warning/6 border border-reap3r-warning/20 rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[12px] text-white">
                <strong>{outdatedAgents.length}</strong> agent{outdatedAgents.length > 1 ? 's' : ''} outdated
              </span>
            </div>
