...
`;
  ...
  fastify.get('/api/install/windows', async (_request, reply) => {
    ...
    const lines: string[] = [
      ...
      `foreach ($d in @($installDir, $dataDirPrimary, $dataDirLegacy, (Join-Path $dataDirPrimary 'logs'), (Join-Path $dataDirLegacy 'logs'))) {`,
      ...
      ``,
      `# Helper: stop a Windows service with timeout, then force-kill if stuck`,
      `function Stop-ServiceSafe {`,
      `  param([string]$Name, [int]$Timeout = 8)`,
      ...
      `  }`,
      `  # Force kill via WMI (works on PS 2.0+ / Server 2008 R2+)`,
      `  try {`,
      `    $wmi = Get-WmiObject Win32_Service -Filter ("Name='" + $Name.Replace("'","''") + "'") -ErrorAction SilentlyContinue`,
      `    if ($wmi -and [int]$wmi.ProcessId -gt 0) {`,
      `      taskkill.exe /PID ([int]$wmi.ProcessId) /F 2>&1 | Out-Null`,
      `    }`,
      `  } catch {}`,
      `}`,
      ``,
      `# 1. Stop and remove service candidates`,
      `foreach ($candidate in $serviceCandidates) {`,
      `  Stop-ServiceSafe -Name $candidate`,
      `  sc.exe delete $candidate 2>&1 | Out-Null`,
      `}`,
      ``,
      `# 2. Scheduled Tasks (cmdlet exists on Win 8+ / Server 2012+)`,
      `try {`,
      `  $hasST = $false`,
      `  try { $hasST = [bool](Get-Command Get-ScheduledTask -ErrorAction SilentlyContinue) } catch {}`,
      `  if ($hasST) {`,
      `    foreach ($candidate in $serviceCandidates) {`,
      `      $t = Get-ScheduledTask -TaskName $candidate -ErrorAction SilentlyContinue`,
      `      if ($t) {`,
      `        Stop-ScheduledTask -TaskName $candidate -ErrorAction SilentlyContinue`,
      ...
      `} catch {}`,
      ``,
      `# 3. Kill any leftover processes by name (use tasklist/taskkill for reliability)`,
      `foreach ($pn in @('reap3r-agent.exe','xefi-agent-2.exe')) {`,
      `  Write-Host "    Checking for process: $pn" -ForegroundColor DarkGray`,
      `  try {`,
      `    $procs = tasklist.exe /FI "IMAGENAME eq $pn" /NH 2>&1 | Where-Object { $_ -match $pn }`,
      `    if ($procs) {`,
      `      Write-Host "    Killing: $pn" -ForegroundColor DarkGray`,
      `      taskkill.exe /F /IM $pn /T 2>&1 | Out-Null`,
      `      Start-Sleep -Milliseconds 500`,
      `    }`,
      `  } catch {}`,
      `}`,
      ...
      `try {`,
      `  $verOut = & $exePath --version 2>&1 | Out-String`,
      ...
      `    & $exePath --enroll --server "$wsUrl" --token "$Token" 2>&1 | Out-Null`,
      ...
      `  Start-Sleep -Milliseconds 500`,
      `  sc.exe create $serviceName binPath= $binPath start= auto DisplayName= "XEFI Agent 2" 2>&1 | Out-Null`,
      `  if ($LASTEXITCODE -ne 0) {`,
      `    Write-Fail "Failed to create service ($serviceName) via sc.exe"`,
      `    Read-Host "Press Enter to exit"`,
      `    exit 1`,
      `  }`,
      `  sc.exe description $serviceName "XEFI Agent 2 remote management service" 2>&1 | Out-Null`,
      `  sc.exe failure $serviceName reset= 0 actions= restart/5000/restart/5000/restart/5000 2>&1 | Out-Null`,
      ...
      `  $svc = $null`,
      `  foreach ($candidate in $serviceCandidates) {`,
      `    $s = Get-Service -Name $candidate -ErrorAction SilentlyContinue`,
      `    if ($s) { $svc = $s; $serviceName = $candidate; break }`,
      `  }`,
