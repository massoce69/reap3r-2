...
`;
  ...
  fastify.get('/api/install/windows', async (_request, reply) => {
    ...
    const lines: string[] = [
      ...
      `# Ignore SSL cert errors (may occur in dev/test environments)`,
      ...
      `  Write-Fail "Please check:"`,
      `  Write-Fail "  - Internet connectivity"`,
      `  Write-Fail "  - Firewall/antivirus blocking HTTPS"`,
      `  Write-Fail "  - Server status (massvision.pro)"`,
      `  Read-Host "Press Enter to exit"`,
      `  exit 1`,
      `}`,
      ``,
