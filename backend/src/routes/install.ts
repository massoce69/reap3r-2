...
`;
  ...
  fastify.get('/api/install/windows', async (_request, reply) => {
    ...
    const lines: string[] = [
      ...
      `function Stop-ServiceSafe([string]$svcName, [int]$TimeoutSec = 12) {`,
      ...
      `  }`,
      `  try {`,
      `    $svcInfo = Get-CimInstance -ClassName Win32_Service -Filter ("Name='" + $svcName.Replace("'","''") + "'") -ErrorAction SilentlyContinue`,
      `    if ($svcInfo -and [int]$svcInfo.ProcessId -gt 0) {`,
      `      Stop-Process -Id ([int]$svcInfo.ProcessId) -Force -ErrorAction SilentlyContinue`,
      `    }`,
      `  } catch {}`,
      `}`,
      `# 1. Stop and remove Service if exists`,
      `foreach ($svcName in $serviceCandidates) {`,
      `  Stop-ServiceSafe -svcName $svcName`,
      `  & sc.exe delete $svcName | Out-Null`,
      `}`,
      `# 2. Stop Scheduled Task if exists (if module available)`,
      ...
      `  }`,
      `}`,
      `# 3. Kill remaining process names`,
