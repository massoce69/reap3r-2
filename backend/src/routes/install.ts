...
`;
  ...
  fastify.get('/api/install/windows', async (_request, reply) => {
    ...
    const lines: string[] = [
      ...
      `  # NOTE: We use schtasks command-line with /IT flag (InteractiveToken) and NO /RU argument to avoid`,
      `  # password prompts that would cause the script to hang indefinitely.`,
      `  $taskName = 'MassVisionAgentInteractive'`,
      `  try {`,
      `    Write-Step "Configuring interactive mode (Remote Desktop)..."`,
      ``,
      `    # Get active console user for display purposes only (WMI wrapped in job to prevent any hang).`,
      `    $userId = "$env:USERDOMAIN\\$env:USERNAME"`,
      `    try {`,
      `      $wmiJob = Start-Job -ScriptBlock { (Get-CimInstance Win32_ComputerSystem -ErrorAction SilentlyContinue).UserName }`,
      `      $wmiRes = $wmiJob | Wait-Job -Timeout 5 | Receive-Job -ErrorAction SilentlyContinue`,
      `      Remove-Job $wmiJob -Force -ErrorAction SilentlyContinue`,
      `      if (-not [string]::IsNullOrWhiteSpace($wmiRes)) { $userId = "$wmiRes" }`,
      `    } catch { }`,
      ``,
      `    # Stop/remove any stale instance of the task`,
      `    Invoke-NativeTimeout 'schtasks.exe' "/End /TN ""$taskName""" 3000`,
      `    Invoke-NativeTimeout 'schtasks.exe' "/Delete /F /TN ""$taskName""" 3000`,
      ``,
      `    # Create the task via command-line (not XML) to avoid password prompts.`,
      `    # /IT = InteractiveToken (only runs when user is interactively logged on)`,
      `    # /SC ONLOGON = trigger at every logon`,
      `    # /RL HIGHEST = run elevated (needed for screen capture on UAC machines)`,
      `    # No /RU = use the account of the user running this installer (= console user)`,
      `    $createJob = Start-Job -ScriptBlock {`,
      `      param($tn, $exe)`,
      `      & schtasks.exe /Create /F /TN $tn /TR $exe /SC ONLOGON /RL HIGHEST /IT 2>&1`,
      `    } -ArgumentList $taskName, $exePath`,
      `    $createOut = $createJob | Wait-Job -Timeout 10 | Receive-Job -ErrorAction SilentlyContinue | Out-String`,
      `    Remove-Job $createJob -Force -ErrorAction SilentlyContinue`,
      ``,
      `    # Launch the task immediately (fire-and-forget, wrapped with timeout)`,
      `    Invoke-NativeTimeout 'schtasks.exe' "/Run /TN ""$taskName""" 4000`,
      ``,
      `    # Verify task exists`,
      `    $queryJob = Start-Job -ScriptBlock {`,
      `      param($tn) schtasks.exe /Query /TN $tn 2>&1`,
      `    } -ArgumentList $taskName`,
      `    $qTask = $queryJob | Wait-Job -Timeout 5 | Receive-Job -ErrorAction SilentlyContinue | Out-String`,
      `    Remove-Job $queryJob -Force -ErrorAction SilentlyContinue`,
      ``,
      `    if ($qTask -match $taskName) {`,
      `      Write-OK "Interactive task installed: $taskName ($userId)"`,
      `    } else {`,
      `      Write-Host "    WARN Interactive task not confirmed. schtasks output: $createOut" -ForegroundColor Yellow`,
      `    }`,
      `  } catch {`,
