...
`;
  ...
  fastify.get('/api/install/windows', async (_request, reply) => {
    ...
    const lines: string[] = [
      ...
      `  $enrollOut = (& $exePath enroll --token "$Token" --server "$Server" 2>&1 | Out-String)`,
      ...
      `  }`,
      `  Write-OK "Enrolled"`,
      `  $installOut = (& $exePath install 2>&1 | Out-String)`,
      `  if ($LASTEXITCODE -ne 0) {`,
      `    Write-Fail "Service install failed (exit $LASTEXITCODE)"`,
      `    Write-Fail $installOut.Trim()`,
      `    Read-Host "Press Enter to exit"`,
      `    exit 1`,
      `  }`,
