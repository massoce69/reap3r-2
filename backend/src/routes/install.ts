...
`;
  ...
  fastify.get('/api/install/windows', async (_request, reply) => {
    ...
    const lines: string[] = [
      ...
      `    # Run task registration in a background job with a hard timeout so this step cannot hang.`,
      `    $job = Start-Job -ArgumentList @($exePath, $taskName, $userId) -ScriptBlock {`,
      `      param($ExePath, $TaskName, $UserId)`,
      ...
      `      throw "Timed out while registering interactive task"`,
      `    }`,
