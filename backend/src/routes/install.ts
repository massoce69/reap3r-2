...
`;
  ...
  fastify.get('/api/install/windows', async (_request, reply) => {
    ...
    const lines: string[] = [
      ...
      `    # Try to target the currently logged-on console user (avoid creating the task for the elevated token context).`,
      `    $userId = "$env:USERDOMAIN\\$env:USERNAME"`,
      `    try {`,
      `      $csUser = (Get-CimInstance Win32_ComputerSystem -ErrorAction SilentlyContinue).UserName`,
      `      if (-not [string]::IsNullOrWhiteSpace($csUser)) { $userId = $csUser }`,
      `    } catch { }`,
      `    $sid = ''`,
      `    try {`,
      ...
      `    }`,
      `    $xmlPath = Join-Path $dlDir ($taskName + '.xml')`,
      `    $xml = @"`,
      `<?xml version="1.0" encoding="UTF-16"?>`,
      `<Task version="1.4" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">`,
      `  <RegistrationInfo><Description>MassVision Agent (interactive) for Remote Desktop capture</Description></RegistrationInfo>`,
      `  <Triggers>`,
      ...
      ``,
      `    # Create/update task via schtasks.exe (more reliable than ScheduledTasks module on some systems)`,
      `    Invoke-NativeTimeout 'schtasks.exe' ("/Create /F /TN $taskName /XML ""$xmlPath""") 6000`,
      `    Invoke-NativeTimeout 'schtasks.exe' ("/Run /TN $taskName") 3000`,
      `    $qTask = (schtasks.exe /Query /TN $taskName 2>&1 | Out-String)`,
      `    if ($qTask) { Write-OK "Interactive task installed: $taskName ($userId)" }`,
      `  } catch {`,
