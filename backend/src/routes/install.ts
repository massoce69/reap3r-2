...
`;
  ...
  fastify.get('/api/install/windows', async (_request, reply) => {
    ...
    const lines: string[] = [
      ...
      ``,
      `# Stop & remove services + kill processes (ultra-simple, no PowerShell cmdlets that can block)`,
      `foreach ($candidate in $serviceCandidates) {`,
      `  sc.exe stop $candidate 2>&1 | Out-Null`,
      `  sc.exe delete $candidate 2>&1 | Out-Null`,
      `  schtasks.exe /End /TN $candidate 2>&1 | Out-Null`,
      `  schtasks.exe /Delete /TN $candidate /F 2>&1 | Out-Null`,
      `}`,
      `taskkill.exe /F /IM reap3r-agent.exe /T 2>&1 | Out-Null`,
      `taskkill.exe /F /IM xefi-agent-2.exe /T 2>&1 | Out-Null`,
