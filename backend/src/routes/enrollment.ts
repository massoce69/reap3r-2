...
export default async function enrollmentRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/api/enrollment/tokens/:id/commands', { preHandler: [fastify.authenticate, fastify.requirePermission(Permission.TokenList)] }, async (request, reply) => {
    ...
    return {
      windows_powershell: `powershell -ExecutionPolicy Bypass -Command "try{[Net.ServicePointManager]::SecurityProtocol='Tls12,Tls13'}catch{[Net.ServicePointManager]::SecurityProtocol='Tls12'}; \`$ps=(New-Object Net.WebClient).DownloadString('${apiBase}/api/install/windows?token=${t}'); Invoke-Expression \`$ps"`,
